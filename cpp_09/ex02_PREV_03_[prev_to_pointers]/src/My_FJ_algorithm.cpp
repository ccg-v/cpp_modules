...
size_t PmergeMe::pairBinarySearch(const std::vector<t_pair> & seq, t_pair value, size_t end, size_t & comparisons) {
    ...
    while (left < right) { 
        ...
        if (seq[mid]._larger > value._larger)
            ...
            left = mid + 1;
		comparisons++;
		std::cout << "\t\t...comparing value " << value._larger << " with " << seq[mid]._larger << " in position [" << mid << "]..." << std::endl;
...
size_t PmergeMe::intBinarySearch(const std::vector<int> & seq, int value, size_t end, size_t & comparisons) {
	size_t partial_comp = comparisons;
    size_t left = 0;
    size_t right = end;
    while (left < right) { 
        ...
        if (seq[mid] > value)
            ...
            left = mid + 1;
		comparisons++;
		std::cout << "\t\t...comparing pending " << value << " with " << seq[mid] << " in position [" << mid << "]... | end is " << seq[right]  << std::endl;
    }
	std::cout << "\t\tcomparisons made = " << comparisons - partial_comp << " | total = " << comparisons << std::endl;
    return left;
}
