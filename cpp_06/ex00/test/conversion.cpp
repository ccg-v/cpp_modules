...
//     const char* decimal = std::strchr(str, '.');
//     const char* flt_max_decimal = std::strchr(flt_max, '.');
//     size_t str_int_len = decimal ? (decimal - str) : std::strlen(str);
//     size_t flt_max_int_len = flt_max_decimal - flt_max;
    
//     if (str_int_len > flt_max_int_len) return true;
//     if (str_int_len < flt_max_int_len) return false;
    
//     // If same number of digits, compare lexicographically
//     int cmp = std::strncmp(str, flt_max, flt_max_int_len);
...
//         decimal++;
//         flt_max_decimal++;
//         while (*decimal && *flt_max_decimal) {
//             if (*decimal > *flt_max_decimal) return true;
//             if (*decimal < *flt_max_decimal) return false;
//             decimal++;
//             flt_max_decimal++;
//         }
//         // If there are more non-zero digits, it's greater
//         while (*decimal) {
//             if (*decimal != '0' && *decimal != 'f' && *decimal != 'F') return true;
//             decimal++;
//         }
//     }
    
//     return false;  // Equal to FLT_MAX is not greater than FLT_MAX
// }

// void toFloat(const char* literal) {
//     if (isGreaterThanFloatMax(literal)) {
...
//     char* end;
//     errno = 0;
//     float f_value = strtof(literal, &end);
